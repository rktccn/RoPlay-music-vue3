import { getTrackDetail, scrobble, getMP3 } from "../apis/track";
...
export default class {
  constructor() {
    ...
    this._repeatMode = "off"; // 循环播放 off | on | 单曲循环 one
    this._shuffle = false; // 随机播放 true | false
    ...
    this._playNextList = []; // (下一首播放列表)当这个list不为空时，会优先播放这个list的歌
  ...
  play() {
    ...
    this._howler?.play();
    this._playing = true;
    document.title = `${this._currentTrack.name} · ${this._currentTrack.ar[0].name} - Ro play`;
  ...
  playCurMusic(TrackID) {
    if (this._list.indexOf(TrackID) !== -1) {
      this.playTrackOnListByID(TrackID);
      return;
    }

    const [ID, index] = this._getNextTrack();
    if (ID === undefined) {
      this._list.push(TrackID);
      this.current = this._list.length;
    } else {
      this._list.splice(index, 0, TrackID);
      this.current = index;
    }
    this._replaceCurrentTrack(TrackID, true, "playNextTrack");
